...
(function(undefined) {
    ...
    ************************************/
    ...
    function Duration(duration) {
        ...
        // it separately.
        this._months = +months + 3 * quarters + 12 * years, this._data = {}, this._locale = moment.localeData(), 
        this._bubble();
    }
    /************************************
        Helpers
    ************************************/
    ...
    function chooseLocale(names) {
        for (var j, next, locale, split, i = 0; i < names.length; ) {
            ...
            next = next ? next.split("-") : null; j > 0; ) {
                ...
                if (next && next.length >= j && compareArrays(split, next, !0) >= j - 1) //the next array item is better than a shallower substring of this one
                break;
                j--;
            }
            i++;
    ...
    function loadLocale(name) {
        ...
        if (!locales[name] && hasModule) try {
            oldLocale = moment.locale(), require("./locale/" + name), // because defineLocale currently also sets the global locale, we want to undo that for lazy loaded locales
    ...
    ************************************/
    ...
    function addTimeToArrayFromToken(token, input, config) {
        ...
        switch (token) {
          ...
          // fall through to MMMM
            case "MMMM":
            a = config._locale.monthsParse(input), // if we didn't find a month name, mark the date as invalid.
          ...
          case "dddd":
            ...
            null != a ? (config._w = config._w || {}, config._w.d = a) : config._pf.invalidWeekday = input;
            break;
    ...
    function dayOfYearFromWeekInfo(config) {
        ...
        w = config._w, null != w.GG || null != w.W || null != w.E ? (dow = 1, doy = 4, // TODO: We need to take the current isoWeekYear, but that depends on
        // how we interpret now (local, utc, fixed offset). So create
        // a now version of current config (take local/utc/offset flags, and
        // create now).
        weekYear = dfl(w.GG, config._a[YEAR], weekOfYear(moment(), 1, 4).year), week = dfl(w.W, 1), 
        weekday = dfl(w.E, 1)) : (dow = config._locale._week.dow, doy = config._locale._week.doy, 
        weekYear = dfl(w.gg, config._a[YEAR], weekOfYear(moment(), dow, doy).year), week = dfl(w.w, 1), 
        null != w.d ? (// weekday -- low day numbers are considered next week
        weekday = w.d, dow > weekday && ++week) : // local weekday -- counting starts from begining of week
        weekday = null != w.e ? w.e + dow : dow), temp = dayOfYearFromWeeks(weekYear, week, weekday, doy, dow), 
    ...
    function dateFromConfig(config) {
        ...
        if (!config._d) {
            ...
            for (currentDate = currentDateArray(config), //compute day of the year from weeks and weekdays
    ...
    function makeDateFromStringAndFormat(config) {
        ...
        // handle am pm
        config._isPm && config._a[HOUR] < 12 && (config._a[HOUR] += 12), // if is 12 am, change hours to 0
        config._isPm === !1 && 12 === config._a[HOUR] && (config._a[HOUR] = 0), dateFromConfig(config), 
    ...
    function makeDateFromStringAndArray(config) {
        ...
        tempConfig._f = config._f[i], makeDateFromStringAndFormat(tempConfig), isValid(tempConfig) && (// if there is any input that was not parsed add a penalty for that format
    ...
    function makeDateFromInput(config) {
        ...
        }), dateFromConfig(config)) : "object" == typeof input ? dateFromObject(config) : "number" == typeof input ? // from milliseconds
        config._d = new Date(input) : moment.createFromInputFallback(config);
    }
    ...
    ************************************/
    ...
    for (/************************************
        Constants
    ************************************/
    var moment, oldGlobalMoment, i, VERSION = "2.8.3", // the global-scope this is NOT the global object in Node.js
    globalScope = "undefined" != typeof global ? global : this, round = Math.round, hasOwnProperty = Object.prototype.hasOwnProperty, YEAR = 0, MONTH = 1, DATE = 2, HOUR = 3, MINUTE = 4, SECOND = 5, MILLISECOND = 6, // internal storage for locale config files
    locales = {}, // extra moment internal properties (plugins register props here)
    momentProperties = [], // check for nodeJS
    hasModule = "undefined" != typeof module && module.exports, // ASP.NET json date format regex
    ...
    ************************************/
    extend(Locale.prototype, {
        ...
        monthsParse: function(monthName) {
            var i, mom, regex;
            for (this._monthsParse || (this._monthsParse = []), i = 0; 12 > i; i++) // test the regex
            if (// make the regex if we don't have it already
            this._monthsParse[i] || (mom = moment.utc([ 2e3, i ]), regex = "^" + this.months(mom, "") + "|^" + this.monthsShort(mom, ""), 
            this._monthsParse[i] = new RegExp(regex.replace(".", ""), "i")), this._monthsParse[i].test(monthName)) return i;
        },
        ...
        weekdaysParse: function(weekdayName) {
            var i, mom, regex;
            for (this._weekdaysParse || (this._weekdaysParse = []), i = 0; 7 > i; i++) // test the regex
            if (// make the regex if we don't have it already
            this._weekdaysParse[i] || (mom = moment([ 2e3, 1 ]).day(i), regex = "^" + this.weekdays(mom, "") + "|^" + this.weekdaysShort(mom, "") + "|^" + this.weekdaysMin(mom, ""), 
    ...
    moment.duration = function(input, key) {
        var sign, ret, parseIso, diffRes, duration = input, // matching against regexp is expensive, do it on demand
        match = null;
    ...
    ************************************/
    ...
    moment.fn.date = makeAccessor("Date", !0), moment.fn.dates = deprecate("dates accessor is deprecated. Use date instead.", makeAccessor("Date", !0)), 
    ...
    moment.fn.toJSON = moment.fn.toISOString, extend(moment.duration.fn = Duration.prototype, {
        _bubble: function() {
            ...
            years += absRound(months / 12), months %= 12, data.days = days, data.months = months, 
            data.years = years;
        },
        abs: function() {
        ...
        as: function(units) {
            ...
            days = this._days + yearsToDays(this._months / 12), units) {
              case "week":
...
function(root) {
    ...
    function repeatZero(qty) {
        ...
        qty = parseInt(qty, 10), !qty || 1 > qty) return result;
        for (;qty; ) result += "0", qty -= 1;
        return result;
    }
    ...
    function padZero(str, len, isRight) {
        return null == str && (str = ""), str = "" + str, (isRight ? str : "") + repeatZero(len - str.length) + (isRight ? "" : str);
    }
    // isArray
    ...
    function findLast(array, callback) {
        for (var index = array.length; index -= 1; ) if (callback(array[index])) return array[index];
    }
    // find
    function find(array, callback) {
        var match, index = 0, max = array.length;
        for ("function" != typeof callback && (match = callback, callback = function(item) {
            return item === match;
        }); max > index; ) {
    ...
    function each(array, callback) {
        ...
        if (array && max) for (;max > index; ) {
            if (callback(array[index], index) === !1) return;
            index += 1;
        }
    }
    // map
    function map(array, callback) {
        ...
        for (;max > index; ) ret[index] = callback(array[index], index), index += 1;
        return ret;
    }
    // pluck
    ...
    function unique(array) {
        var ret = [];
        return each(array, function(_a) {
            find(ret, _a) || ret.push(_a);
        }), ret;
    }
    // intersection
    function intersection(a, b) {
        var ret = [];
        return each(a, function(_a) {
            each(b, function(_b) {
                _a === _b && ret.push(_a);
            });
        }), unique(ret);
    ...
    function rest(array, callback) {
        ...
        return each(array, function(item, index) {
            return callback(item) ? void 0 : (ret = array.slice(index), !1);
        }), ret;
    }
    // initial
    ...
    if (!moment && root.moment && (moment = root.moment), !moment) throw "Moment Duration Format cannot find Moment.js";
    // moment.duration.format([template] [, precision] [, settings])
    moment.duration.fn.format = function() {
        var tokenizer, tokens, types, typeMap, momentTypes, foundFirst, args = [].slice.call(arguments), settings = extend({}, this.format.defaults), // keep a shadow copy of this moment for calculating remainders
        ...
        return settings.duration = this, each(args, function(arg) {
            return "string" == typeof arg || "function" == typeof arg ? void (settings.template = arg) : "number" == typeof arg ? void (settings.precision = arg) : void (isObject(arg) && extend(settings, arg));
        }), types = settings.types = isArray(settings.types) ? settings.types : settings.types.split(" "), 
        "function" == typeof settings.template && (settings.template = settings.template.apply(settings)), 
        tokenizer = new RegExp(map(types, function(type) {
            return settings[type].source;
        }).join("|"), "g"), typeMap = function(token) {
            ...
            return {
                ...
                token: "escape" === type ? token.replace(settings.escape, "$1") : token,
                // ignore type on non-moment tokens
                type: "escape" === type || "general" === type ? null : type
            };
        }, this), momentTypes = intersection(types, unique(compact(pluck(tokens, "type")))), 
        momentTypes.length ? (each(momentTypes, function(momentType, index) {
            var value, wholeValue, decimalValue, isLeast, isMost;
            // calculate integer and decimal value portions
            value = remainder.as(momentType), wholeValue = value > 0 ? Math.floor(value) : Math.ceil(value), 
            ...
            each(tokens, function(token) {
                token.type === momentType && (extend(token, {
                    value: value,
                    wholeValue: wholeValue,
                    decimalValue: decimalValue,
                    isLeast: isLeast,
                    isMost: isMost
                }), isMost && null == settings.forceLength && token.length > 1 && (settings.forceLength = !0));
            }), // update remainder
            remainder.subtract(wholeValue, momentType);
        }), settings.trim && (tokens = ("left" === settings.trim ? rest : initial)(tokens, function(token) {
            // return `true` if:
            // the token is not the least moment token (don't trim the least moment token)
            // the token is a moment token that does not have a value (don't trim moment tokens that have a whole value)
            return !(token.isLeast || null != token.type && token.wholeValue);
        })), foundFirst = !1, "right" === settings.trim && tokens.reverse(), tokens = map(tokens, function(token) {
            var val, decVal;
            if (!token.type) // if it is not a moment token, use the token as its own value
            return token.token;
            // add decimal value if precision > 0
            if (// apply negative precision formatting to the least-significant moment token
            val = token.isLeast && settings.precision < 0 ? (Math.floor(token.wholeValue * Math.pow(10, settings.precision)) * Math.pow(10, -settings.precision)).toString() : token.wholeValue.toString(), 
            // remove negative sign from the beginning
            val = val.replace(/^\-/, ""), // apply token length formatting
            // special handling for the first moment token that is not the most significant in a trimmed template
            token.length > 1 && (foundFirst || token.isMost || settings.forceLength) && (val = padZero(val, token.length)), 
            token.isLeast && settings.precision > 0) switch (decVal = token.decimalValue.toString().replace(/^\-/, "").split(/\.|e\-/), 
            decVal.length) {
              case 1:
                val += "." + padZero(decVal[0], settings.precision, !0).slice(0, settings.precision);
                break;

              case 2:
                val += "." + padZero(decVal[1], settings.precision, !0).slice(0, settings.precision);
                break;

              case 3:
                val += "." + padZero(repeatZero(+decVal[2] - 1) + (decVal[0] || "0") + decVal[1], settings.precision, !0).slice(0, settings.precision);
                break;
            ...
            return token.isMost && token.value < 0 && (val = "-" + val), foundFirst = !0, val;
        }), "right" === settings.trim && tokens.reverse(), tokens.join("")) : pluck(tokens, "token").join("");
        ...
        template: function() {
            var types = this.types, dur = this.duration, lastType = findLast(types, function(type) {
 ...
 */
    ...
    function $CompileProvider($provide, $$sanitizeUriProvider) {
        ...
        var hasDirectives = {}, Suffix = "Directive", COMMENT_DIRECTIVE_REGEXP = /^\s*directive\:\s*([\d\w_\-]+)\s+(.*)$/, CLASS_DIRECTIVE_REGEXP = /(([\d\w_\-]+)(?:\:([^;]+))?;?)/, ALL_OR_NOTHING_ATTRS = makeMap("ngSrc,ngSrcset,src,srcset"), REQUIRE_PREFIX_REGEXP = /^(?:(\^\^?)?(\?)?(\^\^?)?)?/, EVENT_HANDLER_ATTR_REGEXP = /^(on[a-z]+|formaction)$/;
        /**
   * @ngdoc method
        ...
        this.debugInfoEnabled = function(enabled) {
            ...
            function compile($compileNodes, transcludeFn, maxPriority, ignoreDirective, previousCompileContext) {
                ...
                return function(scope, cloneConnectFn, transcludeControllers, parentBoundTranscludeFn, futureParentElement) {
                    assertArg(scope, "scope"), namespace || (namespace = detectNamespaceForChildElements(futureParentElement));
                    var $linkNode;
     ...
     */
            ...
            function createBoundTranscludeFn(scope, transcludeFn, previousBoundTranscludeFn) {
                var boundTranscludeFn = function(transcludedScope, cloneFn, controllers, futureParentElement, containingScope) {
                    ...
                    transcludedScope.$$transcluded = !0), transcludeFn(transcludedScope, cloneFn, controllers, previousBoundTranscludeFn, futureParentElement);
                };
                ...
                switch (nodeType) {
                  case NODE_TYPE_ELEMENT:
                    ...
                    for (var attr, name, nName, ngAttrName, value, isNgAttr, nAttrs = node.attributes, j = 0, jj = nAttrs && nAttrs.length; jj > j; j++) {
                        ...
                        ngAttrName = directiveNormalize(name), (isNgAttr = NG_ATTR_BINDING.test(ngAttrName)) && (name = snake_case(ngAttrName.substr(6), "-"));
     ...
     */
            function applyDirectivesToNode(directives, compileNode, templateAttrs, transcludeFn, jqCollection, originalReplaceDirective, preLinkFns, postLinkFns, previousCompileContext) {
                ...
                function nodeLinkFn(childLinkFn, scope, linkNode, $rootElement, boundTranscludeFn) {
                    ...
                    transcludeFn = boundTranscludeFn && controllersBoundTransclude, controllerDirectives && (// TODO: merge `controllers` and `elementControllers` into single object.
                        ...
                        compile: function() {
                            return {
                                pre: function(scope, element, attr) {
                                    ...
                                    attr[name] && (// we need to interpolate again, in case the attribute value has been updated
                                    // (e.g. by another directive's compile function)
                                    interpolateFn = $interpolate(attr[name], !0, getTrustedContext(node, name), ALL_OR_NOTHING_ATTRS[name] || allOrNothing), 
        ...
        var JSON_START = /^\s*(\[|\{[^\{])/, JSON_END = /[\}\]]\s*$/, PROTECTION_PREFIX = /^\)\]\}',?\n/, APPLICATION_JSON = "application/json", CONTENT_TYPE_APPLICATION_JSON = {
            ...
            transformResponse: [ function(data, headers) {
                if (isString(data)) {
                    ...
                    (contentType && 0 === contentType.indexOf(APPLICATION_JSON) || JSON_START.test(data) && JSON_END.test(data)) && (data = fromJson(data));
                }
                return data;
            } ],
        ...
        this.$get = [ "$rootScope", "$browser", "$sniffer", "$rootElement", function($rootScope, $browser, $sniffer, $rootElement) {
            ...
            return $browser.onUrlChange(function(newUrl, newState) {
                $rootScope.$evalAsync(function() {
                    var oldUrl = $location.absUrl(), oldState = $location.$$state;
                    $location.$$parse(newUrl), $location.$$state = newState, $rootScope.$broadcast("$locationChangeStart", newUrl, oldUrl, newState, oldState).defaultPrevented ? ($location.$$parse(oldUrl), 
                ...
                (initializing || urlOrStateChanged) && (initializing = !1, $rootScope.$evalAsync(function() {
                    $rootScope.$broadcast("$locationChangeStart", $location.absUrl(), oldUrl, $location.$$state, oldState).defaultPrevented ? ($location.$$parse(oldUrl), 
                    $location.$$state = oldState) : (urlOrStateChanged && setBrowserUrlWithFallback($location.absUrl(), currentReplace, oldState === $location.$$state ? null : $location.$$state), 
                    afterLocationChange(oldUrl, oldState));
                })), $location.$$replace = !1;
            }), $location;
        } ];
 ...
 */
    function $ParseProvider() {
        var cache = createMap(), $parseOptions = {
            csp: !1
        };
        this.$get = [ "$filter", "$sniffer", function($filter, $sniffer) {
            ...
            function expressionInputDirtyCheck(newValue, oldValueOfValue) {
                ...
                return null == newValue || null == oldValueOfValue ? newValue === oldValueOfValue : "object" == typeof newValue && (newValue = newValue.valueOf(), 
            ...
            function inputsWatchDelegate(scope, listener, objectEquality, parsedExpression) {
                ...
                if (1 === inputExpressions.length) {
                    ...
                    return inputExpressions = inputExpressions[0], scope.$watch(function(scope) {
                        ...
                        oldInputValue = newInputValue && newInputValue.valueOf()), lastResult;
                ...
                return scope.$watch(function(scope) {
                    for (var changed = !1, i = 0, ii = inputExpressions.length; ii > i; i++) {
                        ...
                        (changed || (changed = !expressionInputDirtyCheck(newInputValue, oldInputValueOfValues[i]))) && (oldInputValueOfValues[i] = newInputValue && newInputValue.valueOf());
                    }
            ...
            return $parseOptions.csp = $sniffer.csp, function(exp, interceptorFn) {
                var parsedExpression, oneTime, cacheKey;
                switch (typeof exp) {
                  case "string":
                    if (cacheKey = exp = exp.trim(), parsedExpression = cache[cacheKey], !parsedExpression) {
 ...
 */
     ...
     */
      ...
      */
       ...
       */
                $digest: function() {
                    var watch, value, last, watchers, length, dirty, next, current, logIdx, logMsg, asyncTask, ttl = TTL, target = this, watchLog = [];
       ...
       */
                $on: function(name, listener) {
                    ...
                    var self = this;
                    return function() {
                        namedListeners[namedListeners.indexOf(listener)] = null, decrementListenerCount(self, 1, name);
                    };
                },
                /**
       * @ngdoc method
 ...
 */
   ...
   */
        this.$get = [ "$document", "$parse", "$sceDelegate", function($document, $parse, $sceDelegate) {
            ...
            if (enabled && $document[0].documentMode < 8) throw $sceMinErr("iequirks", "Strict Contextual Escaping does not support Internet Explorer version < 11 in quirks mode.  You can fix this by adding the text <!doctype html> to the top of your HTML document.  See http://docs.angularjs.org/api/ng.$sce for more information.");
    ...
    function $TemplateRequestProvider() {
        this.$get = [ "$templateCache", "$http", "$q", function($templateCache, $http, $q) {
            function handleRequestFn(tpl, ignoreRequestError) {
                ...
                return self.totalPendingRequests++, $http.get(tpl, {
                    cache: $templateCache
                }).then(function(response) {
                    var html = response.data;
                    return html && 0 !== html.length ? (self.totalPendingRequests--, $templateCache.put(tpl, html), 
                    html) : handleError();
                }, handleError);
            }
...
*/
    function limitToFilter() {
        return function(input, limit) {
            ...
            if (limit = 1/0 === Math.abs(Number(limit)) ? Number(limit) : int(limit), isString(input)) //NaN check on limit
            //NaN check on limit
            return limit ? limit >= 0 ? input.slice(0, limit) : input.slice(limit, input.length) : "";
            var i, n, out = [];
            for (// if abs(limit) exceeds maximum length, trim it
            limit > input.length ? limit = input.length : limit < -input.length && (limit = -input.length), 
    ...
    function orderByFilter($parse) {
        return function(array, sortPredicate, reverseOrder) {
            ...
            sortPredicate = isArray(sortPredicate) ? sortPredicate : [ sortPredicate ], 0 === sortPredicate.length && (sortPredicate = [ "+" ]), 
   ...
   */
    ...
    function baseInputType(scope, element, attr, ctrl, $sniffer, $browser) {
        var placeholder = (element.prop(VALIDITY_STATE_PROPERTY), element[0].placeholder), noevent = {}, type = lowercase(element[0].type);
 ...
 */
   ...
   */
    ...
    Lexer.prototype = {
        ...
        readIdent: function() {
            for (var lastDot, peekIndex, methodName, ch, expression = this.text, ident = "", start = this.index; this.index < this.text.length && (ch = this.text.charAt(this.index), 
            "." === ch || this.isIdent(ch) || this.isNumber(ch)); ) "." === ch && (lastDot = this.index), 
            ident += ch, this.index++;
            //check if this is not a method invocation and if it is back out to last dot
            if (//check if the identifier ends with . and if so move back one char
            lastDot && "." === ident[ident.length - 1] && (this.index--, ident = ident.slice(0, -1), 
            lastDot = ident.lastIndexOf("."), -1 === lastDot && (lastDot = undefined)), lastDot) for (peekIndex = this.index; peekIndex < this.text.length; ) {
 ...
 */
    ...
    Parser.ZERO = extend(function() {
        ...
        filter: function(inputFn) {
            var argsFn, args, token = this.expect(), fn = this.$filter(token.text);
        ...
        ternary: function() {
            ...
            if (token = this.expect("?")) {
                if (middle = this.assignment(), token = this.expect(":")) {
        ...
        logicalAND: function() {
            ...
            return (token = this.expect("&&")) && (left = this.binaryFn(left, token.fn, this.logicalAND(), !0)), 
            left;
        },
        equality: function() {
        ...
        fieldAccess: function(object) {
            var expression = this.text, field = this.expect().text, getter = getterFn(field, this.options, expression);
        ...
        object: function() {
            ...
            if ("}" !== this.peekToken().text) do {
                ...
                break;
                var token = this.expect();
                keys.push(token.string || token.text), this.consume(":");
    ...
    var getterFnCache = createMap(), $sceMinErr = minErr("$sce"), SCE_CONTEXTS = {
        HTML: "html",
        CSS: "css",
        URL: "url",
        ...
        JS: "js"
    }, $compileMinErr = minErr("$compile"), urlParsingNode = document.createElement("a"), originUrl = urlResolve(window.location.href, !0);
 ...
 */
   ...
   */
        ...
        this.$valid = !0, this.$invalid = !1, this.$error = {}, // keep invalid keys here
    ...
    var ngIfDirective = [ "$animate", function($animate) {
        ...
        return {
            ...
            link: function(scope, $element, $attr, ctrl) {
                ...
                }, undefined, undefined, $element)) : ($element.html(ctrl.template), void $compile($element.contents())(scope));
            }
        };
        ...
        */
                    ...
                    function render() {
                        ...
                        var optionGroupName, optionGroup, option, existingParent, existingOptions, existingOption, key, value, groupLength, length, groupIndex, index, selected, lastElement, element, label, optionGroups = {
                            "": []
                        ...
                        for (index = 0; length = keys.length, length > index; index++) key = index, keyName && (key = keys[index], 
                        "$" === key.charAt(0)) || (value = values[key], optionGroupName = callExpression(groupByFn, key, value) || "", 
                        ...
                        label = isDefined(label) ? label : "", optionGroup.push({
                    ...
                    var displayFn = $parse(match[2] || match[1]), valueName = match[4] || match[6], selectAs = / as /.test(match[0]) && match[1], selectAsFn = selectAs ? $parse(selectAs) : null, keyName = match[5], groupByFn = $parse(match[3] || ""), valueFn = $parse(match[2] ? match[1] : valueName), valuesFn = $parse(match[7]), track = match[8], trackFn = track ? $parse(match[8]) : null, // This is an array of array of existing option groups in DOM.
...
function() {
    ...
    angular.module("timeTracker").directive("appMomentInput", function() {
        var VALIDATION_ERROR_KEY = "format", FORMAT_SEPARATOR = "|";
        return {
            require: "ngModel",
            link: function(scope, element, attrs, ngModel) {
                ...
                ngModel.$formatters.push(function(value) {
                    ...
                    return ngModel.$setValidity(VALIDATION_ERROR_KEY, !value || isValidMoment), isValidMoment ? value.format(format[0]) : value;
                }), ngModel.$parsers.push(function(value) {
                    var valueAsMoment = moment(value, format, !0);
                    return ngModel.$setValidity(VALIDATION_ERROR_KEY, !value || valueAsMoment.isValid()), 
                    valueAsMoment.isValid() ? valueAsMoment : value;
                });
            }
        };
    });
    ...
    angular.module("timeTracker").service("timeTrackerEntryRepositoryService", function() {
        var LOCAL_STORAGE_NAME = "TIME_TRACKER_ENTRIES", entries = [], init = function() {
            if (window.localStorage.getItem(LOCAL_STORAGE_NAME)) {
                ...
                for (var index = 0; index < entries.length; index++) {
                    var entry = entries[index], startTime = moment(entry.startTime, moment.ISO_8601, !0);
                    startTime.isValid() && (entry.startTime = startTime);
                    var endTime = moment(entry.endTime, moment.ISO_8601, !0);
                    endTime.isValid() && (entry.endTime = endTime);
                }
            }
        }, now = function() {
            var now = moment();
            return now.second(0), now.millisecond(0), now;
        }, entryRepositoryService = {};
        // todo rename addEmptyEntry
        return entryRepositoryService.persist = function() {
    ...
    angular.module("timeTracker").service("timeTrackerProjectRepositoryService", [ "PROJECT_PERIOD_OF_VALIDITY", function(PROJECT_PERIOD_OF_VALIDITY) {
        var LOCAL_STORAGE_NAME = "PROJECT_ENTRIES", entries = [], init = function() {
            if (window.localStorage.getItem(LOCAL_STORAGE_NAME)) {
                entries = JSON.parse(window.localStorage.getItem(LOCAL_STORAGE_NAME));
                for (var index = 0; index < entries.length; index++) {
                    var entry = entries[index], time = moment(entry.time, moment.ISO_8601, !0);
                    time.isValid() && (entry.time = time);
                }
            }
        }, removeExpiredEntries = function() {
            for (var now = moment(), index = 0; index < entries.length; index++) {
                var entry = entries[index];
                entry.time.diff(now) > PROJECT_PERIOD_OF_VALIDITY && entries.splice(index, 1);
            }
        }, fetchEntryByLabel = function(label) {
            for (var index = 0; index < entries.length; index++) {
                var entry = entries[index];
        ...
        return projectRepositoryService.persist = function() {
            window.localStorage.setItem(LOCAL_STORAGE_NAME, JSON.stringify(entries));
        }, projectRepositoryService.addEntryByLabel = function(label) {
            var entry = fetchEntryByLabel(label);
            entry ? (entry.time = moment(), entry.count++) : (entry = {
                id: Date.now(),
                label: label,
                time: moment(),
                count: 0
            ...
            return removeExpiredEntries(), entries;
        }, init(), projectRepositoryService;
    } ]);
}(), function() {
    "use strict";
    angular.module("timeTracker").service("timeTrackerCalculationService", function() {
        ...
        }, calculateTimeTrackerEntryDuration = function(timeTrackerEntry) {
            var now = moment(), start = timeTrackerEntry.startTime;
            moment.isMoment(start) && start.isValid() || (start = now);
            var end = timeTrackerEntry.endTime;
            return moment.isMoment(end) && end.isValid() || (end = now), moment.duration(end).subtract(start);
        }, findProjectSummaryEntry = function(projectSummaryEntries, project) {
            for (var index = 0; index < projectSummaryEntries.length; index++) {
                var projectSummaryEntry = projectSummaryEntries[index];
                if (projectSummaryEntry.project === project) return projectSummaryEntry;
            }
            return null;
        }, createProjectSummaryEntry = function() {
            return {
                project: "",
                description: "",
        ...
        return calculationService.createProjectEntries = function(timeTrackerEntries) {
            for (var projectEntries = [], index = 0; index < timeTrackerEntries.length; index++) {
                var project = timeTrackerEntries[index].project;
                -1 === projectEntries.indexOf(project) && projectEntries.push(project);
            }
            return projectEntries.sort(), projectEntries;
        }, calculationService.createTimeTrackerSummaryEntries = function(timeTrackerEntries) {
            for (var projectSummaryEntries = [], index = 0; index < timeTrackerEntries.length; index++) {
                ...
                if (isTimeTrackerEntryValid(timeTrackerEntry)) {
                    ...
                    projectSummaryEntry || (projectSummaryEntry = createProjectSummaryEntry(), projectSummaryEntries.push(projectSummaryEntry), 
                    projectSummaryEntry.project = timeTrackerEntry.project), projectSummaryEntry.duration.add(calculateTimeTrackerEntryDuration(timeTrackerEntry)), 
                    timeTrackerEntry.description && (projectSummaryEntry.description += projectSummaryEntry.description ? ", " : "", 
            ...
            for (var duration = moment.duration(0), index = 0; index < timeTrackerEntries.length; index++) {
                var timeTrackerEntry = timeTrackerEntries[index];
                isTimeTrackerEntryValid(timeTrackerEntry) && duration.add(calculateTimeTrackerEntryDuration(timeTrackerEntry));
            }
            return duration;
        }, calculationService;
    });
    ...
    angular.module("timeTracker").service("timeTrackerExportService", function() {
        var exportService = {};
        return exportService.summaryEntriesAsPlainText = function(timeTrackerSummaryEntries) {
            for (var plainText = "", i = 0; i < timeTrackerSummaryEntries.length; i++) {
                var timeTrackerSummaryEntry = timeTrackerSummaryEntries[i];
                plainText += timeTrackerSummaryEntry.project, timeTrackerSummaryEntry.description && (plainText += ": " + timeTrackerSummaryEntry.description), 
    ...
    "use strict";
    angular.module("timeTracker").controller("timeTrackerController", [ "$scope", "$window", "$interval", "timeTrackerEntryRepositoryService", "timeTrackerProjectRepositoryService", "timeTrackerCalculationService", "timeTrackerExportService", function($scope, $window, $interval, timeTrackerEntryRepository, timeTrackerProjectRepositoryService, timeTrackerCalculation, timeTrackerExportService) {
        var now = function() {
            var now = moment();
            return now.second(0), now.millisecond(0), now;
        };
        $scope.timeTrackerEntries = timeTrackerEntryRepository.fetchEntries(), $scope.projectEntries = timeTrackerProjectRepositoryService.fetchEntries(), 
        $scope.appendEmptyTimeTrackerEntry = function() {
            timeTrackerEntryRepository.appendEmptyEntry(), $scope.calculateSummary(), $scope.persist();
        }, $scope.reset = function() {
            timeTrackerEntryRepository.removeAllEntries(), $scope.calculateSummary(), $scope.persist();
        }, $scope.exportSummary = function() {
            var data = timeTrackerExportService.summaryEntriesAsPlainText($scope.timeTrackerSummaryEntries), dataBase64 = $window.btoa(data), dataUrl = 'data:text/plain;charset="utf-8";base64,' + dataBase64;
            $window.open(dataUrl);
        }, $scope.setStartTimeToNow = function(entry) {
            entry.startTime = now(), $scope.calculateSummary(), $scope.persist();
        }, $scope.setEndTimeToNow = function(entry) {
            entry.endTime = now(), $scope.calculateSummary(), $scope.persist();
        }, $scope.removeTimeTrackerEntry = function(entry) {
            timeTrackerEntryRepository.removeEntry(entry.id), $scope.calculateSummary(), $scope.persist();
        }, $scope.persistProject = function(project) {
            project && (timeTrackerProjectRepositoryService.addEntryByLabel(project), timeTrackerProjectRepositoryService.persist(), 
            $scope.projectEntries = timeTrackerProjectRepositoryService.fetchEntries()), $scope.persist();
        ...
        }, $scope.calculateSummary(), $interval($scope.calculateSummary, 1e3);
    } ]);
}(), function() {
    "use strict";
    angular.module("app", [ "timeTracker" ]).constant("PROJECT_PERIOD_OF_VALIDITY", 6912e5);
}();
